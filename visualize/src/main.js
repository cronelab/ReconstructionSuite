...
function initHelpersStack(rendererObj, stack) {
  ...
  let box = {
    ...
    halfDimensions: new Vector3(lpsDims.x + 50, lpsDims.y + 50, lpsDims.z + 50),
  };
    ...
    .then(() => {
      ...
      let load3DBrain_gltf =  () => {
        return new Promise( async (resolve, reject) => {
          ...
          let _electrodeColors = await fetch(`/electrodeColors/${subject}`);
          let electrodeColors;
          let cols = undefined;
          if(_electrodeColors.ok){
            electrodeColors = await _electrodeColors.json()
            cols = electrodeColors[electrodeGroup.name]
          }
          ...
          loader.load(`/electrodes/${subject}`, (object3d) => {
            ...
            object3d.scene.children.forEach((electrodeGroups) => {
              electrodeGroups.children.forEach((electrodeGroup) => {
                electrodeLegend[electrodeGroup.name] =
                  electrodeGroup.children[0].material.color;
                  if(cols!=undefined){
                    electrodeGroup.children[0].material.color = new Color(cols.split(' ')[0],cols.split(' ')[1],cols.split(' ')[2])
                  }
              });
      ...
      load3DBrain_gltf().then((scene) => {
        ...
        let electrodeController = function (legend) {
          ...
          elecs.children[0].children.forEach((elecName) => {
            this[elecName.name] = [
              legend[elecName.name].r * 255,
              legend[elecName.name].g * 255,
              legend[elecName.name].b * 255,
            ];
          });
        };
