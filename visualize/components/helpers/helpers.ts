...
import { orthographicCameraFactory, trackballOrthoControlFactory } from 'ami';
...
export function initRenderer3D(r0, stack) {
  ...
  const lpsDims = new Vector3(
    (worldbb[1] - worldbb[0]) / .5,
    (worldbb[3] - worldbb[2]) / .5,
    (worldbb[5] - worldbb[4]) / .5
  );
  ...
  r0.camera = new CamerasOrthographic(
    -r0.renderer.domElement.clientWidth,
    r0.renderer.domElement.clientWidth,
    r0.renderer.domElement.clientHeight,
    -r0.renderer.domElement.clientHeight,
    0.01,
    10000
  );
