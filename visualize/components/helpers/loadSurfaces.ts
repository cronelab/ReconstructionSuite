...
import {  data, sceneClip, electrodeLegend } from './renderers';
...
export const loadElectrodeScene = async (activeSubject, electrodeColors?) => {
  let electrodes;
  try {
    electrodes = await gltfLoader.loadAsync(`api//electrodes/${activeSubject}`);
...
export const loadBrainScene = async (activeSubject) => {
  let brain;
  try {
    brain = await gltfLoader.loadAsync(`/api/brain/${activeSubject}`);
